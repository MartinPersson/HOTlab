                     ...
                     float * const interAmps)        // intermediate amplitudes (debug)
{
    ...
    // This is technically not required given the cudaMemcpy() below, but having this here ensures
    // that the thread yields and doesn't unnecessarily spin waiting for the kernels to finish.
    cudaDeviceSynchronize();
