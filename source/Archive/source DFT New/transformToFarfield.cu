...
__global__ void transformToFarfield(float *g_x, float *g_y, float *g_z, float *g_pSLM, float *g_aSLM, float *g_Vre, float *g_Vim, int N_spots, unsigned int n, int data_w)
{
	int blockSize = 512;
	...
	{ 
		...
		float aSLM_1 = g_aSLM[i];	
		float p = pSLM_1 - M_PI * (s_zm * (X1*X1 + Y1*Y1) + 2 * (X1 * s_xm + Y1 * s_ym));
		
		s_Vre[tid] += aSLM_1 * cosf(p);
		s_Vim[tid] += aSLM_1 * sinf(p);
