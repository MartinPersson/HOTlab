...
else if (method ==2)			//generate hologram using fast fourier transforms 
{
	...
	cudaMemset(d_FFTd, 0, memsize_SLM_cc);
		cudaThreadSynchronize();		XYtoIndex <<< 1, N_spots >>> (d_x,  d_y, d_spot_index, N_spots, data_w);
		cudaThreadSynchronize();		for (int l=0; l<N_iterations; l++)
	{
